...
export class WebAnalyticsComponent implements OnInit {
  ...
  balance=0;
  amount;
  constructor(private wowService: NgwWowService, private zone: NgZone,private http: HttpClient,private authService:AuthService,private payService: PayService,
    ...
    this.authService.getUserById(user.id).subscribe((user)=>{
      this.balance=Number(user.balance);
      console.log(this.balance);});

   /*  Observable.interval(3000).subscribe(x => { // will execute every 30 seconds
      let userr=this.authService.getUserDetails();
      this.authService.getUserById(userr.id).subscribe((user)=>{
        this.balance=Number(user.balance);
        console.log(this.balance);});
  ...
  pay(){
    ...
    let user=this.authService.getUserDetails();
    let info={price:this.amount,userId: user.id};
    console.log(info);
    this.payService.pay(info).subscribe((data)=>{console.log(data['forwardLink']);
    window.location.href=(data['forwardLink']).toString()
